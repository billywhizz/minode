...
var FileCache = minode.filecache.FileCache;
...
function SocketServer(config) {
  ...
  var fc;
  if(config.filecache) {
    fc = new FileCache(config.filecache);
  }
  if(!config.hasOwnProperty("unmask")) {
    config.unmask = true;
  }
  if(!config.hasOwnProperty("decode")) {
    config.decode = true;
  }
    ...
    peer.onRequest = function(request) {
      if(request.upgrade) {
        ...
        peer.onWSStart = function() {
          ...
          peer.parser.decode = config.decode;
          peer.parser.unmask = config.unmask;
