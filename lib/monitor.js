...
function psmonitor(interval, fn) {
  ...
  fs.stat("/proc/" + process.pid + "/stat", function (err, stats) {
    ...
    fs.open("/proc/" + process.pid + "/stat", "r", parseInt(438,8), function(err, fd) {
      if(err) {
        fn(err);
        return;
      }
      fs.open("/proc/stat", "r", parseInt(438,8), function(err, fd2) {
        if(err) {
          fn(err);
          return;
        }
...
function netmonitor(interval, fn) {
  ...
  fs.open("/proc/net/dev", "r", parseInt(438,8), function(err, fd) {
    if(err) {
     fn(err);
     return;
    }
