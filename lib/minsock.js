...
// TODO - don't think i ever need to call shutdown unless i want to close only one end of the connection
      peer.close();
      peer.closed = true;
      if(peer.onclose) {
        peer.onclose();
  ...
  function setSecure(options) {
    ...
    if(!options.buffers) {
      options.buffers = {
        secure: new Buffer(8*1024*1024),
        clear: new Buffer(8*1024*1024)
      };
    }
