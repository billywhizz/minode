  ...
  c.onConnection = function(peer) {
    ...
    peer.onResponse = function(info) {
      ...
      peer.onWSStart = function() {
        ...
        _binary = (_websocket.binaryType.toLowerCase()==="arraybuffer");
        return true;
      };
    ...
    _websocket.send = function(data) {
      var r;
      if(_binary) {
        r = peer.sendMessage(data, null, function(status, handle, req) {
          if(status !== 0) {
    ...
    if(url.protocol === "ws:") {
      ...
      peer.onSecure = function(err) {
        if(_websocket.onSecure) {
          // override default behaviour
          if(!_websocket.onSecure(err, peer)) {
            _websocket.onerror(err);
            return;
          }
        }
